...
/**
 * WhatSalesTogether component displays a report of menu items sold together within a specified date range.
 * @returns {JSX.Element} WhatSalesTogether component JSX.
 */
...
function WhatSalesTogether({}) {
  ...
  function generateReport() {
    ...
    const url = `${process.env.REACT_APP_WEB_SERVER_ADDRESS}/sales_together_report?start_date=${startDateFormatted}&end_date=${endDateFormatted}`;
    ...
    fetch(url)
      ...
      .then((data) => {
        ...
        const labels = data.map(
          (item) => item.menu_item_name1 + "/" + item.menu_item_name2
        );
        const counts = data.map((item) => parseInt(item.frequency));
